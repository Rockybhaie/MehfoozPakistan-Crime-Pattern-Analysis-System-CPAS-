...
const WitnessProfile = () => {
  ...
  const loadProfile = async () => {
    try {
      setLoading(true);
      const response = await witnessAPI.getProfile();
      console.log('👤 Witness profile loaded:', response.data);
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      await witnessAPI.updateProfile(formData);
      Swal.fire({
        icon: 'success',
        ...
        <div className="page-header" style={{ marginBottom: '30px' }}>
          <div>
            ...
            <p style={{ color: '#666', margin: '5px 0 0 0' }}>
              View and update your witness information
            </p>
          </div>
          {!editing && (
            <button 
          ...
          <div style={{
            ...
            <div style={{
              ...
              <h3 style={{ margin: '0 0 5px 0', fontSize: '24px', color: '#333' }}>
                {formData.name || 'Witness User'}
              </h3>
              <p style={{ margin: 0, color: '#666', fontSize: '14px' }}>
                Witness ID: #{profile?.WITNESS_ID || 'N/A'}
              </p>
            </div>
